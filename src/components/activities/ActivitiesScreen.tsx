  ...
  return (
    <div className="min-h-screen bg-[#F0F6FF] text-[#172033] pb-36 pt-4 sm:pt-6">
      <div className="max-w-xl mx-auto px-4 space-y-5">
        ...
        {/* Tab Switcher */}
        <div className="flex items-center gap-2 border-b border-[#DCE8F7] pb-2">
          <button
            onClick={() => setActiveTab('my_posts')}
            className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${
              activeTab === 'my_posts'
                ? 'bg-[#2563EB] text-white shadow-md shadow-blue-500/20'
                : 'bg-white text-[#64748B] border border-[#DCE8F7] hover:text-[#172033] shadow-xs'
            }`}
          >
        ...
        {activeTab === 'my_posts' ? (
          <div className="space-y-4">
            {myPostedRequests.length === 0 ? (
              <div className="bg-white border border-[#DCE8F7] rounded-3xl p-8 text-center space-y-3 shadow-xs">
                <p className="text-sm font-bold text-[#172033]">No requests posted yet</p>
              ...
              myPostedRequests.map(req => (
                <div
                  key={req.id}
                  className="bg-white border border-[#DCE8F7] rounded-3xl p-5 shadow-xs space-y-4"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#DCE8F7] pb-3">
                    <div className="min-w-0 flex-1">
                      <span className="px-2.5 py-0.5 rounded-full bg-[#F8FBFF] text-[#2563EB] border border-[#DCE8F7] text-[10px] font-black uppercase tracking-wide">
                        {req.category}
                      </span>
                      <h3 className="text-base font-black text-[#172033] mt-1">{req.title}</h3>
                      <p className="text-xs text-[#64748B]">
                        {req.date} • {req.time} • {req.location}
                      </p>
                    </div>

                    <div className="text-xs font-bold text-[#172033] bg-[#F8FBFF] px-3 py-1.5 rounded-xl border border-[#DCE8F7] shrink-0 self-start sm:self-center">
                  ...
                  <div className="space-y-2.5">
                    <h4 className="font-extrabold text-xs text-[#64748B] uppercase tracking-wider">
                      ...
                      <div className="space-y-2">
                        {req.interestedUsers.map(u => {
                          ...
                          return (
                            <div
                              key={u.id}
                              className="bg-[#F8FBFF] border border-[#DCE8F7] p-3 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                            >
                              <div
                              ...
                              >
                                <img
                                  src={u.avatar}
                                  alt={u.name}
                                  className="w-9 h-9 rounded-full object-cover ring-2 ring-white group-hover/applicant:ring-[#2563EB] transition-all shrink-0"
                                />
                                <div className="min-w-0 flex-1">
                                  <h5 className="font-bold text-xs sm:text-sm text-[#172033] group-hover/applicant:text-[#2563EB] transition-colors truncate">{u.name}</h5>
                                  {u.note && <p className="text-xs text-[#64748B] italic line-clamp-2">"{u.note}"</p>}
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                                {u.status === 'ACCEPTED' ? (
                                  ...
                                  <>
                                    <button
                                      ...
                                      className="p-1.5 rounded-full bg-white hover:bg-rose-50 text-[#64748B] hover:text-rose-500 border border-[#DCE8F7]"
                                      title="Decline"
                                    >
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-full bg-white hover:bg-[#F0F6FF] text-[#64748B] hover:text-[#2563EB] border border-[#DCE8F7]"
          ...
          <div className="space-y-4">
            {myJoinedRequests.length === 0 ? (
              <div className="bg-white border border-[#DCE8F7] rounded-3xl p-8 text-center space-y-2 shadow-xs">
                <p className="text-sm font-bold text-[#172033]">No joined requests</p>
                <p className="text-xs text-[#64748B]">
              ...
              myJoinedRequests.map(req => (
                <div
                  key={req.id}
                  className="bg-white border border-[#DCE8F7] rounded-3xl p-5 shadow-xs flex items-center justify-between gap-4"
                >
                  <div className="space-y-1">
                    <span className="px-2.5 py-0.5 rounded-full bg-[#F8FBFF] text-[#2563EB] text-[10px] font-black uppercase border border-[#DCE8F7]">
                      {req.category}
                    </span>
                    <h3 className="text-base font-black text-[#172033]">{req.title}</h3>
                    <p className="text-xs text-[#64748B]">
